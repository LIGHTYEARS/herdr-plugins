...
enum Request {
    ...
    Diff {
        root: PathBuf,
        rel: String,
        /// "staged" | "worktree" | "untracked" | "committed:<base_oid>:<head_oid>".
        kind: String,
    },
...
/// A committed preview accepts only pinned, complete object IDs. A malformed
/// control request must not become a Git revision expression (or silently turn
/// into a working-tree preview).
fn committed_diff_oids(kind: &str) -> Option<(&str, &str)> {
    let rest = kind.strip_prefix("committed:")?;
    let (base, head) = rest.split_once(':')?;
    let valid = |oid: &str| {
        matches!(oid.len(), 40 | 64) && oid.bytes().all(|byte| byte.is_ascii_hexdigit())
    };
    (valid(base) && valid(head)).then_some((base, head))
}
...
fn load_diff(root: &Path, rel: &str, kind: &str) -> Doc {
    ...
    let what = match kind {
        ...
        _ => {
            if let Some((base, head)) = committed_diff_oids(kind) {
                args.push(base.into());
                args.push(head.into());
                "committed"
            } else {
                return Doc {
                    name,
                    context: format!("{} — invalid diff request", root.join(rel).display()),
                    lines: vec![Line::raw("(invalid diff request)")],
                    numbered: false,
                    media: None,
                    scroll: 0,
                    wrap: true,
    ...
    let lines = match output {
        ...
        Ok(out) => {
            ...
            let text = String::from_utf8_lossy(&out.stdout);
            if !out.status.success() && !(kind == "untracked" && out.status.code() == Some(1)) {
                let err = String::from_utf8_lossy(&out.stderr);
                vec![Line::raw(format!(
                    "(git diff failed: {})",
                    if err.trim().is_empty() {
...
mod tests {
    ...
    #[test]
    fn committed_preview_uses_pinned_commits_and_rejects_unknown_kinds() {
        use std::sync::atomic::{AtomicU64, Ordering};
        static NEXT: AtomicU64 = AtomicU64::new(0);
        let root = std::env::temp_dir().join(format!(
            "herdr-committed-preview-{}-{}",
            std::process::id(),
        ...
        std::fs::write(root.join("file.txt"), "middle\n").unwrap();
        run(&["commit", "-qam", "middle"]);
        let head = run(&["rev-parse", "HEAD"]);
        std::fs::write(root.join("file.txt"), "latest\n").unwrap();
        run(&["commit", "-qam", "latest"]);
        std::fs::write(root.join("file.txt"), "dirty\n").unwrap();
        let kind = format!("committed:{base}:{head}");
        assert_eq!(
            committed_diff_oids(&kind),
            Some((base.as_str(), head.as_str()))
        );
        let doc = load_diff(&root, "file.txt", &kind);
        let body = doc
            .lines
            .iter()
            .map(|line| {
        ...
        assert!(
            !body.contains("latest") && !body.contains("dirty"),
            "{body}"
        );
        for invalid in [
            "committed:HEAD:HEAD",
            ...
            assert_eq!(committed_diff_oids(invalid), None);
            let doc = load_diff(&root, "file.txt", invalid);
            assert_eq!(doc.lines[0].spans[0].content, "(invalid diff request)");
        }
        std::fs::remove_dir_all(root).unwrap();
    }

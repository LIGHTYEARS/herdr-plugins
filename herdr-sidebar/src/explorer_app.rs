...
static WORKSPACE_HINT_SHOWN: AtomicBool = AtomicBool::new(false);

fn workspace_delta_hint(root: &Path) -> Option<String> {
    let selected = workspace::discover(root).ok().flatten()?;
    let count = selected
        .folders
        .iter()
        .filter(|folder| folder.task.is_some())
        .count();
    if count < 2 || WORKSPACE_HINT_SHOWN.swap(true, Ordering::Relaxed) {
        return None;
    }
    let declared: usize = selected.declared.values().sum();
    Some(format!(
        "Δ Workspace · {count}/{declared} selected · D open"
    ))
}
...
impl App {
    pub fn new(
        ...
        let workspace_hint = workspace_delta_hint(&tree.root_path());
        let mut app = Self {
            tree,
            rows,
    ...
    pub fn on_key(&mut self, key: KeyEvent) -> Option<Exit> {
        ...
        if key.code == KeyCode::Char('D')
            ...
            && workspace::discover(&self.tree.root_path())
                .ok()
                .flatten()
                .is_some()
        {
            return Some(Exit::WorkspaceDelta);
        }
        self.notice = None;
    ...
    pub fn on_mouse(&mut self, mouse: MouseEvent) -> Option<Exit> {
        ...
        if mouse.kind == MouseEventKind::Down(MouseButton::Left)
            && self
                .notice
                .as_deref()
                .is_some_and(|notice| notice.starts_with("Δ Workspace"))
            && mouse.row
                >= self
                    .last_height
                    .saturating_sub(self.footer_height(self.last_width))
            && mouse.column < self.last_width.saturating_sub(3)
        {
            return Some(Exit::WorkspaceDelta);
        }
        if mouse.kind == MouseEventKind::Down(MouseButton::Left) {
